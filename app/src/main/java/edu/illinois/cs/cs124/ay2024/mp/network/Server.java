...
public final class Server extends Dispatcher {
  ...
  private final Map<String, RSO> rsos = new HashMap<>();

  private final Map<String, Boolean> favMap = new HashMap<>();
  ...
  private MockResponse getRSO(String path) throws JsonProcessingException {
    ...
    if (id.endsWith("/") || !(path.startsWith("/rso/"))) {
      return HTTP_NOT_FOUND;
    }

    if (rsos.containsKey(id)) {
      return makeOKJSONResponse(OBJECT_MAPPER.writeValueAsString(rsos.get(id)));
    } else {
      return HTTP_NOT_FOUND;
    }
    //What happens if the RSO object doesn't exist
    //return the serialization RSO object to the client

  }

  private MockResponse getFavorite(String path) throws JsonProcessingException {
    // Split path
    String[] parts = path.split("/");

    String id = parts[2];

    if (!parts[1].equals("favorite") || !favMap.containsKey(id)) {
      return HTTP_NOT_FOUND;
    }

    Favorite fav = new Favorite(id, favMap.get(id));

    try {
      return makeOKJSONResponse(OBJECT_MAPPER.writeValueAsString(fav));
    } catch (JsonProcessingException e) {
      throw new RuntimeException(e);
    }
  }

  private MockResponse postFavorite(@NonNull RecordedRequest request) {
    String id = "";

    try {
      Favorite data = OBJECT_MAPPER.readValue(request.getBody().readUtf8(), Favorite.class);
      boolean input = data.getFavorite();
      id = data.getId();
    ...
    return new MockResponse()
        ...
        .setHeader("Location", "/favorite/" + id);
  }
  ...
  public MockResponse dispatch(@NonNull RecordedRequest request) {
    logger.log(Level.INFO, "Server receiving client requests");
    ...
    try {
      if (path.isEmpty() && method.equals("GET")) {
        ...
        for (String k : favMap.keySet()) {
          favMap.put(k, false);
        }
        return makeOKJSONResponse("200: OK");
      } else if (path.equals("/summary") && method.equals("GET")) {
        logger.log(Level.INFO, "Client is requesting list of RSO Summaries");
        return getSummaries();
      } else if (path.startsWith("/rso") && method.equals("GET")) {
        return getRSO(path);
      } else if (path.startsWith("/favorite") && method.equals("GET")) {
        return getFavorite(path);
      } else if (path.startsWith("/favorite") && method.equals("POST")) {
        return postFavorite(request);
      } else if (path.startsWith("/testpost")) {
        logger.log(Level.INFO, method);
        String contents = request.getBody().readUtf8();
        logger.log(Level.INFO, contents);
        if (method.equals("POST")) {
          return new MockResponse()
              .setResponseCode(HttpURLConnection.HTTP_MOVED_TEMP)
              .setHeader("Location", "/testpost");
        } else {
  ...
  private void loadData() {
    ...
    try {
      ...
      for (JsonNode node : nodes) {
        ...
        rsos.put(rso.getId(), rso);
        Summary summary = new Summary(rsoData);
        summaries.add(summary);

        favMap.put(rso.getId(), false);
      }
